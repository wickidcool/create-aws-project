...
<task type="auto">
  ...
  <action>
Update template token system to support organization account IDs:
...
3. In src/templates/manifest.ts, update deriveTokenValues function:
   - Add org tokens derivation from config.org:
     ...
     - DEV_ACCOUNT_ID: find account with environment 'dev' and return accountId or ''
     - STAGE_ACCOUNT_ID: find account with environment 'stage' and return accountId or ''
     - PROD_ACCOUNT_ID: find account with environment 'prod' and return accountId or ''

These tokens will be available in templates as {{ORG_NAME}}, {{DEV_ACCOUNT_ID}}, etc.
Templates can use conditional blocks: // {{#if ORG_ENABLED}}...{{/if ORG_ENABLED}}
  </action>
  <verify>npm run build succeeds. npm test passes (token derivation tests may need updates).</verify>
  <done>Organization tokens defined and derived from config. Account IDs available for template substitution.</done>
</task>
...
- Template tokens available for {{ORG_NAME}}, {{DEV_ACCOUNT_ID}}, etc.
